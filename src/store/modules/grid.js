...
const actions = {
    ...
    async setColPattern({ state }, { row, pattern }) {
        ...
        for (let col = 0; col < dim; col++) {
            ...
            let preveCell = colPattern[col-1];
            if (colPattern[col-1]) {
                if (typeof cell === "string" && typeof preveCell.data === "number")
                    colPattern[col-1].data = preveCell.data+1
                if (typeof cell === "number" && typeof preveCell.data === "string")
                    cell += 1;
            }
